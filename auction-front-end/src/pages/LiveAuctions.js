        ...
        axios.get(`${apiConfig.auctions}`)
            .then(response => {
                if (Array.isArray(response.data)) {
                    // Lọc các phiên đấu giá đang hoạt động
                    const activeAuctions = response.data
                        .filter(auction => auction.status === "active")
                        .sort((a, b) => new Date(a.auctionEndTime) - new Date(b.auctionEndTime)); // Sắp xếp theo thời gian kết thúc
                                ...
                                <Link to={`/auction/${auction.auctionId}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                                    <Card className="h-100">
                                        <Card.Img
                                            ...
                                            alt={auction.product?.name || 'Không có tên'}
                                        ...
                                        <Card.Body>
                                            ...
                                            <Card.Text style={{ fontSize: '1rem', fontWeight: 'bold', color: '#007bff' }}>
                                                {auction.currentPrice ? auction.currentPrice.toLocaleString('vi-VN') + ' VNĐ' : 'Đang cập nhật'}
                                            </Card.Text>
                                            <Card.Text
                                                style={{
                                                    fontSize: '0.85rem',
